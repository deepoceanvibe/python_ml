...
# 데이터 분할
x = data['Hipertension', 'Diabetes', 'Alcoholism', 'Handcap']
y = data['No-show']
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.7, random_state=42)

# 데이터 전처리
data = data[['No-show', 'Hipertension', 'Diabetes', 'Alcoholism', 'Handcap']]
...
data['Handcap'] = data['Handcap'].map({'Yes': 1, 'No': 0})

data.fillna(-1, inplace=True)

# 상관관계 분석
correlation = data[['Hipertension', 'Diabetes', 'Alcoholism', 'Handcap', 'No-show']].corr()['No-show']
print(correlation)
